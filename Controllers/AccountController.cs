...
namespace Bloomie.Controllers
{
   public class AccountController : Controller
   {
       ...
       public AccountController(UserManager<ApplicationUser> userManager, SignInManager<ApplicationUser> signInManager, RoleManager<IdentityRole> roleManager, ApplicationDbContext context, IEmailService emailService)
       {
...
[HttpGet]
       ...
       public async Task<IActionResult> EnableTwoFactor(bool enable)
       {
           ...
           else
           {
               ...
               TempData["success"] = "Xác thực hai bước đã được tắt.";
       ...
       public async Task<IActionResult> TwoFactorLogin(TwoFactorViewModel model)
       {
           ...
           var result = await _signInManager.TwoFactorSignInAsync(TokenOptions.DefaultEmailProvider, model.TwoFactorCode, isPersistent: false, rememberClient: false);
           if (result.Succeeded)
           {
       ...
       public async Task<IActionResult> Login(LoginViewModel loginVM)
       {
           ...
           var result = await _signInManager.PasswordSignInAsync(loginVM.UserName, loginVM.Password, isPersistent: false, lockoutOnFailure: true);
               if (result.Succeeded)
               {
       ...
       public async Task<IActionResult> GoogleResponse()
       {
           ...
           if (existingUser == null)
           {
               ...
               $"<a href='{Request.Scheme}://{Request.Host}/Account/SetNewPassword?email={newUser.Email}&token={token}'>nhấn vào đây</a> để đặt mật khẩu. Liên kết này sẽ hết hạn sau 1 tiếng.");
